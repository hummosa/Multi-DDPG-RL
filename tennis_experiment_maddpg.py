...
class Config():
    def __init__(self):
        self.state_size=  env_info.vector_observations.shape[1]   + brain.vector_action_space_size
...
agent1 = Agent(state_size=config.state_size, action_size=config.action_size, random_seed=1, config=config)
agent2 = Agent(state_size=config.state_size, action_size=config.action_size, random_seed=0, config=config)
...
def ddpg(n_episodes=300, max_t=1000):
    ...
    for i_episode in tqdm(range(1, n_episodes+1)):
        ...
        for t in range(max_t):
            ...
            with torch.no_grad():
                actions1 = agent1.act(combine_state_action(states[0]))
                actions2 = agent2.act(combine_state_action(states[1]))
            actions = np.hstack([actions1, actions2])
            env_info = env.step(actions)[brain_name]
            
            next_states = env_info.vector_observations
            with torch.no_grad():
                next_actions1 = agent1.act(combine_state_action(next_states[0]))
                next_actions2 = agent2.act(combine_state_action(next_states[1]))
            ...
            agent1.step(combine_state_action(states[0], actions2), actions1, np.expand_dims(rewards1,0), combine_state_action(next_states[0], next_actions2), np.expand_dims(dones[0],0))
            agent2.step(combine_state_action(states[1], actions1), actions2, np.expand_dims(rewards2,0), combine_state_action(next_states[1], next_actions1), np.expand_dims(dones[1],0))
            states = next_states
